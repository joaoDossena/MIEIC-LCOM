...
int write_cmdbyte(uint32_t cmd)
{
  ...
  while (1) {
    if (write_kbc(STAT_REG, KBC_WRITECMDBYTE) == 1) {
      printf("unable to write KBC_WRITECMDBYTE\n");
      return 1;
    }

    if (write_kbc(OUT_BUF, cmd) == 1) {
      printf("unable to write cmd in OUT_BUF\n");
      return 1;
    }
  ...
  switch(st_machine->currentState)
  {
    ...
    case VERTEX: //At vertex of inverted V state
      if(pp->mb) //If the middle button is pressed
        ...
        m_ev->type = LB_PRESSED;
      else if(pp->rb) //If only the right button is pressed
