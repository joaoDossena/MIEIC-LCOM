	...
	uint16_t rec_width = h_res / no_rectangles;
  	...
  	for (unsigned int line = 0; line < no_rectangles; line++)
  	{
    	for (unsigned int rec = 0; rec < no_rectangles; rec++)
    	{
      		...
      		if (vg_draw_rectangle(rec_width*rec, rec_height*line, rec_width, rec_height, current_color) != 0)
        		return 1;
    	}
  	}
  	return 0;
...
int draw_pix_map(xpm_image_t img, uint16_t x, uint16_t y)
{
	...
	{
		if ((unsigned) (y+i) >= v_res)
			break;

		for (int j = 0; j < img.width; j++)
		{
			if ((unsigned) (x+j) >= h_res)
				break;

			if (color_pixel(x+j, y+i, img.bytes[j+i*img.width]) != 0)
				return 1;
		}
	}
	return 0;
...
/*int vbe_get_ctrl_info(vg_vbe_contr_info_t *contr_info, vbe_info_t* info)
{
  return 1
}
*/
